      ...
      <SidebarBody className="sticky top-24">
        <div className="space-y-1 relative">
          ...
          {allLessons.map((lesson, index) => {
            ...
            // Icon based on lesson completion and quest status
            let icon;
            if (lesson.isQuest) {
              icon = lesson.completed ? 
                ...
                <div className="w-6 h-6 border-2 border-amber-400 rounded-full z-10 flex-shrink-0 bg-amber-50 flex items-center justify-center text-xs font-bold text-amber-600 absolute left-0">
                  <Trophy size={12} />
                </div>;
            } else {
              icon = lesson.completed ? 
            ...
            return (
              <SidebarLink
                ...
                } ${lesson.isQuest ? "border-l-2 border-amber-400" : ""}`}
              />
            );
          })}
        </div>
